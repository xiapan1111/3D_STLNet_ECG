...
from configs.Config_Ribeiro_Model import config_net
...
def train(config):
    ...
    for fold_index in range(10):
        ...
        scheduler_lr_net = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=config.patience, min_lr=config.learning_rate/100)
        ...
        for epoch in range(1, config.max_epoch + 1):
            train_loss, train_inputs, train_targets, train_outputs, train_auc = train_epoch(epoch, model, optimizer,
                                                                                            ...
                                                                                            use_cuda)
            val_loss, val_targets, val_outputs, val_auc = test_epoch(epoch, model, criterion, val_dataloader, use_cuda)
            ...
            scheduler_lr_net.step(val_loss)
...
if __name__ == '__main__':
    ...
    for exp in ['exp3', 'exp1.1', 'exp1.1.1']:
        if exp == 'exp3':  # 'rhythm'
            ...
            config_net.seed = 2023

        config_net.experiment = exp
        train(config_net)

    # CPSC 2018 task
    config_net.datafolder = '../data/CPSC/'
    config_net.experiment = 'cpsc'
    config_net.seed = 2023
    print("batch_size", config_net.batch_size)
    print("max_epoch", config_net.max_epoch)
    print("learning_rate", config_net.learning_rate)
    print("patience", config_net.patience)
    train(config_net)

    # HFHC task
    config_net.datafolder = '../data/hf/'
    config_net.experiment = 'hf'
    config_net.seed = 9
    train(config_net)
