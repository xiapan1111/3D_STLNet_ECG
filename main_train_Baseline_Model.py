...
from configs.Config_Baseline_Model import config_net
...
def train(config):
    ...
    for fold_index in range(10):
        ...
        train_dataloader, val_dataloader, test_dataloader, num_classes = load_datasets(
            datafolder=config.datafolder, experiment=config.experiment, batch_size=config.batch_size,
            train_fold=train_fold + 1, val_fold=val_fold + 1, test_fold=test_fold + 1)
        ...
        scheduler_lr_net = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[10, 20, 30, 40],
